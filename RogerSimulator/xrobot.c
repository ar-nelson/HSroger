...
/**           version of Roger-the-Crab - ENV2 (long room)     **/
/** author:   Grupen                                           **/
/** date:     April, 2010                                      **/
/****************************************************************/
#include <stdio.h>
...
// put all of this in header file project.h
...
// ENV2 should disallow JOINT ANGLE (0) inputs and permit only Cartesian
//  BASE GOAL(1), ARM GOAL(2), BALL POSITION(3), and MAP EDITOR (4) modes
...
XtPointer client_data, call_data;
{
  ...
  switch (Roger.input_mode) {
    // ENV2 remove case for JOINT input
...
XtPointer client_data, call_data;
{
  ...
  switch (Roger.control_mode) {
    // ENV2 no longer need control mode for motor units
     case PROJECT1:
...
Robot * roger;
{
  ...
  // put ENV2-ROOMS directory in place with 3 rooms available
  switch (roger->room_num) {
     case R0:
       fp = fopen("ROOMS-ENV2/R0-64x160.txt", "r");
       break;
     case R1:
       fp = fopen("ROOMS-ENV2/R1-64x160.txt", "r");
       break;
     case R2:
       fp = fopen("ROOMS-ENV2/R2-64x160.txt", "r");
       break;
     default:
       break;
  }
  ...
  // ENV2 - need 2.5 times as many BINS in the x direction as ENV1
  for (i=0; i<NYBINS; i++) {
    fgets(line, NXBINS+2, fp);
    for (j=0; j<NXBINS; j++) {
...
void draw_all() 
{
  ...
  draw_potential_maps(); // ENV2 onlt Cartesian potential maps
  draw_boundaries();     // ENV2 update
  draw_frames();         // ENV2 update

  switch (Roger.control_mode) {

  case PROJECT2: // ArmKinematics (Cartesian arm control)
    ...
    break;
  case PROJECT3: // Vision (oculomotor track)
    draw_object(objects[TOY]);
    draw_roger(mobile_base, arms, eyes);
    if (draw_visual==TRUE) {
  ...
  case PROJECT4: // SearchTrack
    ...
    break;
  case PROJECT5: // Stereo Triangulation
    draw_object(objects[TOY]);
    draw_roger(mobile_base, arms, eyes);
    if (draw_visual==TRUE) {
  ...
  case PROJECT7: // CHASEPUNCH
    ...
    break;
  case PROJECT8: // PATHPLANNING --- this is the only one that is different?
...
#define FRAME_T 0.045
  ...
  // ENV2 - convert these to Cartesian panel and label left and right images
  XDrawString(display, pixmap, gc, 
	      W2DX(zoom,-0.75), W2DY(zoom,-3.5), "left     /", 9);
  XDrawString(display, pixmap, gc, 
	      W2DX(zoom,-0.20), W2DY(zoom,-3.5), "image", 5);
  ...
  // ENV2 - convert these to Cartesian panel and label left and right images
  XDrawString(display,pixmap,gc,
	      W2DX(zoom,-0.85),W2DY(zoom,-3.5), "right    /", 10);
  XDrawString(display, pixmap, gc, 
	      W2DX(zoom,-0.15), W2DY(zoom,-3.5), "image", 5);
...
void draw_boundaries()
{
  ...
  XSetForeground(display, gc, foreground);
  XDrawLine(display, pixmap, gc, 
	    W2DX(zoom,MIN_X), W2DY(zoom,MAX_Y), // ENV2 - change MAX and MIN X
...
// draw the NYBINSxNXBINS potential map on the canvas
...
int color;
{
  ...
  int i, row, col, current_row, current_col;
  int already_used[NYBINS][NXBINS], count;
  ...
  for (row=1;row<(NYBINS-1); ++row) {
    for (col=1;col<(NXBINS-1); ++col) {
...
void make_images()
{
  ...
  int sort[NYBINS*NXBINS];
  VisibleObject vobject[NYBINS*NXBINS];
...
void goal_detection()
{
  ...
  if (toy.position[Y] < (MIN_Y + R_GOAL)) {
    ...
    XDrawString(display, pixmap, gc, 
		W2DX(zoom,-0.45), W2DY(zoom,-3.6), "time=", 5);
...
Robot * roger;
{
  ...
  if (roger->button_event) {
    switch(roger->input_mode) {
      // ENV2 - don't need Joint Angle Input mode 
      //    case JOINT_ANGLE_INPUT:
  ...
  switch(roger->control_mode) {
     ...
     case PROJECT9:
       ...
       break;
       //     case PROJECT10:
       //       project10_control(roger, simtime);
       //       break;
       //     case PROJECT11:
       //       project11_control(roger, simtime);
       //       break;
     default:
       break;
  }
...
int argc;char **argv;
{
  ...
  // default for ENV2 is "Input: Base goal"
  input_mode_w = widget = 
