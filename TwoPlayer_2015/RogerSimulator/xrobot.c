...
PolyBall objects_1[NBODY];  // NBODY=5: BASE, ARM1, ARM2, OBJECT, occupancy grid
...
void initialize_pong_object()
{
	...
	object.extForce[X] = object.extForce[Y] = object.extForce[THETA]= 0.0;
}
...
void read_interface()
{
 // pass back torques (write only)
  arms_1[0][1].torque = 
    motor_model(Roger_1.arm_torque[0][0], Roger_1.arm_theta_dot[0][0],
		SHOULDER_TS, SHOULDER_W0);
  arms_1[1][1].torque = 
    motor_model(Roger_1.arm_torque[1][0], Roger_1.arm_theta_dot[1][0],
		SHOULDER_TS, SHOULDER_W0);
  arms_1[0][2].torque = 
    motor_model(Roger_1.arm_torque[0][1], Roger_1.arm_theta_dot[0][1],
		ELBOW_TS, ELBOW_W0);
  arms_1[1][2].torque = 
    motor_model(Roger_1.arm_torque[1][1], Roger_1.arm_theta_dot[1][1],
		ELBOW_TS, ELBOW_W0);

  eyes_1[0].torque = 
    motor_model(Roger_1.eye_torque[0], Roger_1.eye_theta_dot[0], EYE_TS, EYE_W0);
  eyes_1[1].torque = 
    motor_model(Roger_1.eye_torque[1], Roger_1.eye_theta_dot[1], EYE_TS, EYE_W0);

  mobile_base_1.wheel_torque[0] = 
    motor_model(Roger_1.wheel_torque[0], Roger_1.wheel_theta_dot[0],
		WHEEL_TS, WHEEL_W0);
  mobile_base_1.wheel_torque[1] = 
    motor_model(Roger_1.wheel_torque[1], Roger_1.wheel_theta_dot[1],
		WHEEL_TS, WHEEL_W0);

	// pass back torques (write only)
  arms_2[0][1].torque = 
    motor_model(Roger_2.arm_torque[0][0], Roger_2.arm_theta_dot[0][0],
		SHOULDER_TS, SHOULDER_W0);
  arms_2[1][1].torque = 
    motor_model(Roger_2.arm_torque[1][0], Roger_2.arm_theta_dot[1][0],
		SHOULDER_TS, SHOULDER_W0);
  arms_2[0][2].torque = 
    motor_model(Roger_2.arm_torque[0][1], Roger_2.arm_theta_dot[0][1],
		ELBOW_TS, ELBOW_W0);
  arms_2[1][2].torque = 
    motor_model(Roger_2.arm_torque[1][1], Roger_2.arm_theta_dot[1][1],
		ELBOW_TS, ELBOW_W0);

  eyes_2[0].torque = 
    motor_model(Roger_2.eye_torque[0], Roger_2.eye_theta_dot[0], EYE_TS, EYE_W0);
  eyes_2[1].torque = 
    motor_model(Roger_2.eye_torque[1], Roger_2.eye_theta_dot[1], EYE_TS, EYE_W0);

  mobile_base_2.wheel_torque[0] = 
    motor_model(Roger_2.wheel_torque[0], Roger_2.wheel_theta_dot[0],
		WHEEL_TS, WHEEL_W0);
  mobile_base_2.wheel_torque[1] = 
    motor_model(Roger_2.wheel_torque[1], Roger_2.wheel_theta_dot[1],
		WHEEL_TS, WHEEL_W0);
...
void compute_external_forces(base, arms, obj, Roger, objects)
Base * base;
...

{
  ...
  void copy_object(), update_objects();//, sim_fwd_kinematics(), sim_arm_Jacobian();
  ...
  for (i=0; i<(NBODY-1); ++i) { // compute force on body i by body j
    for (j=(i+1); j<NBODY; ++j) {
      ...
      copy_object(i, &obji, objects); copy_object(j, &objj, &objects);
  ...
  // BASE
  base->extForce[X] = objects[BASE].net_extForce[X];
  base->extForce[Y] = objects[BASE].net_extForce[Y];
  ...
  // TOY OBJECT
  obj->extForce[X] = objects[TOY].net_extForce[X];
  obj->extForce[Y] = objects[TOY].net_extForce[Y];
  obj->extForce[THETA] = objects[TOY].net_extForce[THETA];
...
void update_objects(mobile_base, arms, obj, objects)
Base * mobile_base;
Arm arms[NARMS][NARM_FRAMES];
Obj * obj;
PolyBall objects[NBODY];
{
  ...
  /****************** 3: TOY OBJECT - CIRCLE || TRIANGLE *********************/
     objects[TOY].id = toy_home.id;
     objects[TOY].N =  toy_home.N;
     objects[TOY].Rsphere = toy_home.Rsphere;
     objects[TOY].radius = toy_home.radius;
     objects[TOY].mass = toy_home.mass;
     objects[TOY].moi = toy_home.moi;

  objects[TOY].position[X] = obj->position[X]; 
  objects[TOY].position[Y] = obj->position[Y];
  objects[TOY].position[THETA] = obj->position[THETA];

  objects[TOY].velocity[X] = obj->velocity[X];
  objects[TOY].velocity[Y] = obj->velocity[Y];
  objects[TOY].velocity[THETA] = obj->velocity[THETA];
}
...
Obj * obj;
{
/*	// flesh out the objects array based on the   
	PolyBall objects[NBODY];
	update_objects(&base, &arms, &obj, &objects);
	compute_external_forces(&base, &arms, &obj, &Roger_1, &objects);
*/
...
Eye eyes[NEYES];
{
  ...
  r_b[0] = R_BASE/2.0; r_b[1] = -R_BASE-WHEEL_THICKNESS; r_b[2] = 0.0; r_b[3] = 1.0;
  SIMmatXvec(mobile_base.wTb, r_b, r_w);
  draw_circle(W2DX(zoom, r_w[0]), W2DY(zoom, r_w[1]), W2DR(zoom,WHEEL_THICKNESS),FILL);
  r_b[0] = -R_BASE/2.0; r_b[1] = -R_BASE-WHEEL_THICKNESS; r_b[2] = 0.0; r_b[3] = 1.0;
  SIMmatXvec(mobile_base.wTb, r_b, r_w);
  draw_circle(W2DX(zoom, r_w[0]), W2DY(zoom, r_w[1]), W2DR(zoom,WHEEL_THICKNESS),FILL);
...
void new_game() {
	...
	simulate_roger(&mobile_base_1, arms_1, eyes_1);
	simulate_roger(&mobile_base_2, arms_2, eyes_2);
